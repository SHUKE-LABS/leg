...
//!
//! [`crate::events`] describes a single provider *call* (the `baton.exchange/v1`
//! record). This module adds the distinct contract for a *peer message*: who
//! it is from/to, which conversation and turn it belongs to, what kind of
//! message it is, and its body.
//!
...
//! with the owned [`crate::events::Exchange`] value (its `request` + terminal
...
/// [`Exchange`](crate::events::Exchange) value so the nesting is explicit on the
...
mod tests {
    use super::*;
    use crate::events::{Outcome, RequestRecord};
